...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Input: Annotated recording ----
      selectizeInput("native", "¿Eres un hablante nativo de la idioma en la grabación?",
                   choices = c("Sí", "No"),
                   options = list(
      ...
      # Input: Last chance ----
      selectizeInput("evaltype", "¿Qué tipo de prueba querés llevar a cabo con tu archivo?",
                   choices = c("Normal", "Última oportunidad"),
                   options = list(
                     placeholder = 'Seleccioná una opción',
                     onInitialize = I('function() { this.setValue(""); }'))),
...
server <- function(input, output) {
  ...
  tagList(
      ...
      tags$div("Tus puntajes ponderados para non-CHI diarización y xds son:"),
